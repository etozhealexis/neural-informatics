...
spread = 0.1;
%задаем вероятностную сеть
net = newpnn([trainX;trainY], T, spread);

display(net);

% view(net);
%прямое кодирование классов
Y = sim(net, [trainX; trainY]);
%перевод вектора в индексы преобразуют в 123
Yc = vec2ind(Y);
%вычисляем разницу
Z = abs(Yc-traint);

N = size(traint, 2);
% если разница между выходом и эталоном 0, то прибавяем количество верно
% классиф точек и вычиляем процент
p = 0;
